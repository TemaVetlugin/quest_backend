...
class UpdateController extends Controller
{
    ...
    public function __invoke(Request $request, Quest $quest)
    {
        ...
        if($request->file('file_start')){
            ...
            $data['file'] = Storage::disk('public')->putFile('/quests', $file_start);
        }
        if($request->file('file_main')){
            ...
            $data['file'] = Storage::disk('public')->putFile('/quests', $file_main);
        }
        if($request->file('file_end')){
            ...
            $data['file'] = Storage::disk('public')->putFile('/quests', $file_end);
        }
        $quest->update($data);
